...
export default function CompanyImport({ onCompanyAdded }) {
  ...
  return (
    ...
    >
      <h2 className="text-sm font-semibold mb-1 uppercase tracking-wide" style={{ color: 'var(--text)' }}>
        Import from Apollo
      </h2>
      <p className="text-xs mb-4" style={{ color: 'var(--text-subtle)' }}>
        Search a real company database and import directly
      </p>

      <form onSubmit={handleSearch} className="flex flex-col gap-2 mb-4">
        <input type="text" value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Search query..." className={inputClass} style={fieldStyle} />
        <div className="flex gap-2">
          <input type="text" value={country} onChange={(e) => setCountry(e.target.value)} placeholder="Country (optional)" className={`${inputClass} flex-1`} style={fieldStyle} />
            ...
            return (
              ...
              >
                <div className="flex flex-col gap-0.5 min-w-0">
                  <span className="font-medium text-sm" style={{ color: 'var(--text)' }}>{company.name}</span>
                  <div className="flex gap-2 text-xs flex-wrap" style={{ color: 'var(--text-subtle)' }}>
